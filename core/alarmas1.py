...
class SistemaAlarmas(QObject):
    ...
    def guardar_config(self):
        config = {"nombres": self.nombres, "limites": self.limites, "niveles": self.niveles, "tipos": self.tipos, "trigger_booleano": self.trigger_booleano}
        try:
    ...
    def cargar_config(self):
        if os.path.exists("config.json"):
            try:
                with open("config.json", "r", encoding="utf-8") as f: self.configurar(**json.load(f))
            except: pass
