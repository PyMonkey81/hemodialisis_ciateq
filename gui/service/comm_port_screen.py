...
class CommPortScreen(QWidget):
    ...
    def _update_port_exclusion(self):
        """Exclusión mutua entre los 4 combos"""
        ...
        for port, combos in [
            (main_port, [self.cmb_cond_port, self.cmb_mega_port, self.cmb_bioz_port]),
            (cond_port, [self.cmb_main_port, self.cmb_mega_port, self.cmb_bioz_port]),
            (mega_port,  [self.cmb_main_port, self.cmb_cond_port, self.cmb_bioz_port]),
            (bioz_port, [self.cmb_main_port, self.cmb_cond_port, self.cmb_mega_port]),
        ]:
            if port != "Auto":
                for combo in combos:
    ...
    def _repopulate_combos(self):
        ...
        self._block_signals(True)
        try:
            for combo in [self.cmb_main_port, self.cmb_cond_port, self.cmb_mega_port, self.cmb_bioz_port]:
                combo.clear()
    ...
    def _sanitize_platform_ports(self, settings: dict):
        for sensor_key in ("main_control", "conductivity_sensor", "mega_conductivity_sensor", "bioz_urea_sensor"):
