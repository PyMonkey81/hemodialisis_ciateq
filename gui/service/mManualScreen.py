...
# Asegúrate de que estas importaciones sean correctas en tu estructura de proyecto
# from core.variables_map import VARIABLES  <-- Descomenta si tienes este archivo
# from gui.components.LED import LED        <-- Descomenta si tienes este componente
# from gui.components.ToggleSwitch import ToggleSwitch <-- Descomenta si tienes este componente

# MOCKS para que el código sea ejecutable sin las dependencias externas (solo para prueba)
try:
    from core.variables_map import VARIABLES, ANALOG_MAP
except ImportError:
    VARIABLES = {0x01: {}, 0x02: {}} # Mock vacío

# Si no tienes los componentes LED o ToggleSwitch a mano, define clases dummy o importa las reales.
# Asumo que las tienes, así que mantengo las importaciones originales arriba, 
# pero añado un try/except para evitar errores si copias y pegas esto solo.
try:
    from gui.components.LED import LED
    from gui.components.ToggleSwitch import ToggleSwitch
except ImportError:
    # Clases dummy por si faltan los archivos
    class LED(QWidget):
...
class mManualScr(QWidget):
    ...
    def escribir_setpoint(self, tag, widget_input):
        try:
            texto = widget_input.text().replace(',', '.')
            if not texto: 
                # Si el campo se vacía y se pierde el foco, restaurar valor
                # Esto es opcional, pero mejora la UX si el usuario borra todo accidentalmente
            ...
            for group_key, variables_in_group in VARIABLES.items():
                # Nos aseguramos de que sea un diccionario de variables para ese grupo
                # y que no sea una entrada global si VARIABLES tiene una estructura compleja
            ...
            if found and target_group != -1 and target_id != -1:
                ...
                if VARIABLES[target_group][target_id].get("rw", False):
                    ...
                    if self.parent_window and hasattr(self.parent_window, 'serial'):
                        # ¡Llamar a escribir_double con el grupo y el ID de la variable!
                        ...
                        print(f"[MOCK] Serial no conectado. Simulación de escritura para {tag}: Grupo {hex(target_group)}, ID {target_id}, Valor {valor}")
                else:
        ...
        if direccion != -1:
            if self.parent_window and hasattr(self.parent_window, 'serial') and self.parent_window.serial:
                try:
                    ...
                    print(f"[MOCK] Simulación de envío: Addr {direccion} Val {estado}")
            else:
                 print(f"[MOCK] Simulación de envío (Sin serial): Addr {direccion} Val {estado}")
        else:
            print(f" -> Error: No se encontró ID para el tag '{tag}'")
