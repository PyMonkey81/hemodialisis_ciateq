...
#         cols = ["#", "Grupo", "Nombre", "Tipo", "Valor", "Límites", "Unidad", "Estado", "R/W"]
...
#         #     QTableWidget { gridline-color: #cbd5e1; font-size: 13px; background: #000000; }
#         #     QHeaderView::section { background: #1e40af; color: #ffffff; padding: 10px; font-weight: bold; }
#         # """)
#         self.tabla.setStyleSheet("""
#         QTableWidget {
...
#         # Diccionario para mapear nombre_variable -> (QTableWidgetItem de valor, QTableWidgetItem de estado)
#         # Esto hace la actualización instantánea sin borrar filas
#         self.celdas_valor = {}
#         self.celdas_estado = {}
        
#         self.construir_tabla() # Construimos la estructura solo UNA vez

#         self.timer = QTimer(self)
#         self.timer.timeout.connect(self.actualizar_datos)
#         # 500ms es suficiente, 1000ms a veces se siente lento
...
#         # Iteramos una sola vez para crear la estructura
...
#                 self.tabla.setItem(row, 3, QTableWidgetItem("DOUBLE" if tipo == "double" else "BOOL"))
                
#                 # Items dinámicos (los guardamos para actualizar luego)
#                 item_valor = QTableWidgetItem("---")
#                 item_valor.setTextAlignment(Qt.AlignCenter)
#                 self.tabla.setItem(row, 4, item_valor)
#                 self.celdas_valor[nombre] = (item_valor, tipo) # Guardamos referencia y tipo

#                 self.tabla.setItem(row, 5, QTableWidgetItem(limites_str))
#                 self.tabla.setItem(row, 6, QTableWidgetItem(unidad))
                
#                 item_estado = QTableWidgetItem("NORMAL")
#                 item_estado.setTextAlignment(Qt.AlignCenter)
#                 self.tabla.setItem(row, 7, item_estado)
#                 self.celdas_estado[nombre] = item_estado # Guardamos referencia

#                 self.tabla.setItem(row, 8, QTableWidgetItem(rw))
#                 idx += 1

#     def actualizar_datos(self):
#         """Solo actualiza los textos y colores, no reconstruye la tabla."""
#         if not self.isVisible(): 
#             return # Ahorra CPU si no se ve

#         # Recuperar sistema de alarmas si se perdió
#         alarmas = self.sistema_alarmas or (self.parent().sistema_alarmas if hasattr(self.parent(), 'sistema_alarmas') else None)

#         for nombre, (item_val, tipo) in self.celdas_valor.items():
#             # 1. Actualizar Valor
#             raw_val = self.valores.get(nombre, 0.0)
#             if tipo == "double":
#                 texto = f"{raw_val:.2f}"
#             else:
#                 texto = "ON" if raw_val > 0 else "OFF"
            
#             # Solo actualizamos si cambió el texto (optimización Qt)
#             if item_val.text() != texto:
#                 item_val.setText(texto)

#             # 2. Actualizar Estado (Alarmas)
#             estado_texto = "NORMAL"
...
#             if alarmas:
#                 try:
#                     # Buscamos si este nombre es una alarma
#                     if nombre in alarmas.nombres:
#                         idx_alarma = alarmas.nombres.index(nombre)
...
#                 except:
#                     pass # Evitar crash por incoherencia momentánea en listas

#             item_est = self.celdas_estado[nombre]
